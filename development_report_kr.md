...
chat.py는 사용자에게 대화 인풋 값을 받아 변수에 저장하고, 변수에 저장한 사용자의 말을 토큰 단위로 분석해 자신이 가지고 있는 기존의 베이스 데이터인 data.pth와 비교하여 적절한 답변을 내놓는다. 다음 코드 진행에서 챗봇은 사용자가 입력한 입력값 토큰으로부터 사용자가 하고자 하는 말을 데이터베이스와 딥러닝에 기반해서 예측하고, 자신이 생각하기에 가장 적절한 답변을 json 데이터베이스의 response에서 랜덤하게 인출한다. 이 때, 사용자의 토큰에 대한 정보가 없거나 기존 데이터베이스에서 적절한 답변을 찾을 수 없을 경우 'It's not philosophical...'이라는 오류 문구를 return하게끔 프로그래밍하였다.

* 단, train.py를 사전에 실행시키지 않아 data,pth가 없거나 data.pth가 비어 있는 경우 챗봇은 제대로 동작하지 않고 아웃풋 값을 내놓지 않는다.
* 만약 intents.json 파일 데이터베이스에 변화를 주었다면 train.py를 통해 재학습시켜야 새로운 데이터셋이 대화에 반영된다.

여기까지가 대화를 할 수 있는 인공지능 챗봇을 구축하는 데 필요한 코드들이다. 그리고 마지막으로 사용한 코드는 이 인공지능 시스템을 로컬 웹에서 사용할 수 있도록 구현하는 작업에 사용되는 프로그램 코드들이다. main에서 static 파일과 templates에 담겨 있는 파일들이 그것이며, 각각 채팅 아이콘을 위한 이미지 파일 하나와 코드 파일 2개, 전반적인 로컬 웹 디자인을 담은 html 파일 하나로 이루어져 있다. 코드에는 app.js, style.css가 존재하며, app.js는 웹에서의 채팅 시스템을 실행하기 위한 스크립트 파일이, style.css는 폰트 등의 택스트 정보가 담겨 있다. 그리고 마지막으로 base.html은 본 프로그램을 로컬 웹에서 구동했을 때의 전체적인 홈페이지 디자인 양식을 위한 코드들이 들어 있다.
